 ...
 *  Returns	:	Function result depends on cndition available in the
 *functions.
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    if (std::strstr(path.c_str(), search_video_device.c_str()) != NULL) {
      ...
      if (check_camera_type(device_node, &productName, &type) != FAILURE) {
        if (type == USB) {
          pdev = udev_device_get_parent_with_subsystem_devtype(dev, "usb",
                                                               "usb_device");
 ...
 *****************************************************************************/
int Devices::check_camera_type(std::string &dev_node, std::string *productName,
                               int *type) {
  int cam_fd;
  std::string name;
  struct v4l2_capability cam_cap;
 ...
 *****************************************************************************/
int Devices::get_camera_count() { return camera_name.size(); }
 ...
 *****************************************************************************/
bool Devices::get_camera(int index, std::string *cam_name,
 ...
 *****************************************************************************/
std::vector<std::string> Devices::get_list() { return camera_name; }
} // namespace vio_cam
